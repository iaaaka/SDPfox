...
public class DistanceForLigandStatistics {
	...
	private static SDPDistStatForDiffMethods getResults(InputParameters p) throws Exception{
		...
		SDPDistStatForDiffMethods result = new SDPDistStatForDiffMethods(p.name,allDist.length);
	...
	private static double getPvalueForMinValue(double[] allDist,double[] sdpDist,double minSDPdist){
		double result = 1;
		int betterSDP = 0;
	...
	private static double getWilcoxonByR(double[] allDist, double[] sdpDist)throws Exception{
	...
	private static double[] getDistancesForSDP(InputParameters p, int method) throws Exception {
		...
		int[] sdps = null;
		if(p.name.equals("PF00132") && p.seqName.equals("THGA_ECOLI/95-112")){
			sdps = getSDPFromInputFile(OTHER_METHODS + p.name + "-1." +SDPDistStatForDiffMethods.EXTENSIONS[method]);
		}else{
			sdps = getSDPFromInputFile(OTHER_METHODS + p.name + "." +SDPDistStatForDiffMethods.EXTENSIONS[method]);
		}
	...
	private static double[][] getDistancesBetweenResiduesAndLigand(InputParameters p)throws Exception {
		...
		int[] sdps = getSDPforSeq(ALIGNMENTS + p.name
	...
	private static void makePvalueByMonteCarlo(int monteCarloIteration) throws Exception {
		...
		for (int i = 0; i < pars.size(); i++) {
			int[] sdps = getSDPforSeq(ALIGNMENTS + pars.get(i).name
			...
			System.out.println(pars.get(i).name);
			if(sdps == null){
				pw.println(pars.get(i).name + "_" + pars.get(i).startindex + "\tsdp don`t exist");
				continue;
			}
			...
			pw.println(pars.get(i).name + "_" + pars.get(i).startindex + "\t"
					+ result);
			pw.flush();
		}
		pw.close();
	...
	private static void makeInputFile(InputParameters p) throws Exception {
		...
		saveInputFile(OUTPUT + p.name + "_" + p.startindex, sdps, p.ligands,
	...
	private static Vector<InputParameters> readInputParameters(String fname)
			throws Exception {
		BufferedReader bf = new BufferedReader(new FileReader(fname));
	...
	private static void saveInputFile(String name, int[] sdps, int[] ligands,
